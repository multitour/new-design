...
(function($){
    ...
    Tooltip.prototype = {
        ...
        place:function(element, $tooltip, e){
            ...
            x=(x+tooltipmeasure.w > windowmeasure.left+windowmeasure.w-3)? x-tooltipmeasure.w+elmeasure.w+20 : x; //right align tooltip if no space to the right of the element
            y = y+arrowheight;

            var isrightaligned = x != elmeasure.left; //Boolean to indicate if tooltip is right aligned

            var arrowpos=(isrightaligned)? tooltipmeasure.w-(elmeasure.left+elmeasure.w-e.pageX) : e.pageX-elmeasure.left; //25 is to move arrow 25px to the left so it's not obscured by cursor
