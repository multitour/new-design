...
//            autoSizable: true, // Indicate if the content is auto sizable. If not, the min size will be used
        ...
        $('.nyroModal1').live("click",function(e) {
            e.preventDefault();    
            link=$(this).attr("rel");
            $.nyroModalManual({
                url: link, 
		...
		preloadImg('/files/out/nyromodal-1.6.2/ajaxLoader.gif');
		preloadImg('/files/out/nyromodal-1.6.2/prev.gif');
		preloadImg('/files/out/nyromodal-1.6.2/next.gif');
    ...
    $(function() {
      $('#fotogallery1').nyroModal({
            width: 750, // default Width If null, will be calculate automatically
            height: 700, // default Height If null, will be calculate automatically
            minWidth: 400, // Minimum width
            minHeight: 300 // Minimum height
      });
    });
    $(function() {
      $('#fotogallery2').nyroModal({
            width: 750, // default Width If null, will be calculate automatically
            height: 700, // default Height If null, will be calculate automatically
            minWidth: 400, // Minimum width
            minHeight: 300 // Minimum height
...
$(document).ready(function(){
    $("a.linkwindow").click(function(){
        //linkhref=$(this).attr("href");
        linkrel=$(this).attr("rel");
        //document.location.href = href;
        //alert(linkrel);
        if (linkrel) $(this).attr("href",linkrel);
        // if (linkrel) document.location.href=linkrel;
        // return false;
    });
});    
...
//            resizable: true, // Indicate if the content is resizable. Will be set to false for swf
 ...
 var scrollBot = $('body').height();
 var bot1 = scrollBot-470;
 var bot2 = scrollBot-395;
 $('.hotel-position-bot1').css( {top: bot1, position: 'absolute', display: 'block',font: 'normal 12px tahoma'} );
 $('.hotel-position-bot2').css( {top: bot2, position: 'absolute', display: 'block',font: 'normal 12px tahoma', margin: '0 0 0 -305px'} );
 });

$(document).ready(function()
{
    var $big_image = $('#big_image');
    $big_image.click(function(){loadModal($big_image.attr('loadurl'), this.title);});

    $('.image_preview').click(function()
    {
        $('td.big_image_td #bimgMap').css({display:'none'});
        var link = $(this).parent().attr('href');
        var title = this.alt;
		$big_image.attr({src: $(this).attr('load'), title:title}).css({display:'block'});
        if (link) $big_image.unbind('click').click(function(){
            loadModal(link, title);
        });
	});

	var content_div  = $('.photo_galery_content');
    if (!content_div[0]) return false;
	var content_table = $('.photo_galery_content_table');
    if (!content_table[0]) return false;
    var $scroll_strip = $(".sсroll_strip");
    var stripPosition = $scroll_strip.position();
    var scroll_button = $(".scroll_button", $scroll_strip);
    if (!scroll_button[0]) return false;
	var def_scroll_button_left_position = stripPosition.left;
    scroll_button.css({left:def_scroll_button_left_position});
	var step = 97;
	var slide_length = parseInt($scroll_strip.width());
    //print_r($scroll_strip.position());
	var content_table_width = content_table.width();
	var scroll_button_width = scroll_button.width();
	var new_left_position = 0;
	var count_images_in_screen = 3;
	var slide_length_width = slide_length - scroll_button_width;
	var move_button_width = 5;	
	var slider_screen_width;
	var current_scroll_button_left_position;
	var max_scroll_button_right_position = stripPosition.left + slide_length - scroll_button_width;
	var max_content_table_left_position = -679;
	
	slider_screen_width = count_images_in_screen * (step - move_button_width);
	
	scroll_button.draggable({drag: function(event, ui)
    {
			current_scroll_button_left_position = ui.absolutePosition.left;
			
			new_left_position = (current_scroll_button_left_position - def_scroll_button_left_position);
			var y = new_left_position * (content_table_width - slider_screen_width) / slide_length_width;
			new_left_position = y * -1;
			//ew_left_position = y;
			//alert(new_left_position);
			content_table.css('left', new_left_position);
			//content_div.css('padding-right', new_left_position);
		}
	});
	
	scroll_button.draggable('option', 'axis', 'x');
	scroll_button.draggable('option', 'containment', 'parent');

	
	$('.slide_image').click(function()
    {
		var slider_step, slide_button_left;
		var scroll_button_left_position;
		
		scroll_button_left_position = $('.scroll_button').position().left;
		slider_step =  slide_length_width * step / (content_table_width-slider_screen_width);
		
		var max_left_value;
		max_left_value = (content_table_width/step) - 2;
		max_left_value = (step * max_left_value) * -1;
		...
		if (to == 'left')
        {
            ...
            {
				if ((new_left_position + step) < 0) {
					new_left_position += step;
					slide_button_left = scroll_button_left_position - slider_step;
				} else {
					new_left_position = 0;
					slide_button_left = def_scroll_button_left_position;
				}
				
				content_table.css('left', new_left_position);
				scroll_button.css('left', slide_button_left);
			} else {
				return false;
			}
		} else if (to == 'right')
        {
			if (max_left_value < (new_left_position-step))
            {
				new_left_position -= step;
                /*
				if (new_left_position < max_content_table_left_position) {
					new_left_position = max_content_table_left_position;
				}*/

				content_table.css('left', new_left_position);
				
				
				slide_button_left = slider_step + scroll_button_left_position;
				
				if (slide_button_left > max_scroll_button_right_position) {
					slide_button_left = max_scroll_button_right_position;
				}
				
				scroll_button.css('left', slide_button_left);
				//alert(slide_button_left);
			}	
		}
	});
});